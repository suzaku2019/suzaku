...
int mds_kv_set(const char *key, const void *value, uint32_t valuelen)
{
        ...
        snprintf(path, MAX_LINE_LEN, "%s/mds/%s",  SHM_ROOT, key);
...
static int __mds_kv_get_file(const char *path, void *entry, uint32_t *valuelen)
{
        int ret, len;
        char buf[1024 * 64];
...
static int __mds_kv_get_dir(const char *path, uint64_t offset, void *de0, uint32_t *valuelen)
{
        int ret, size, buflen;
	struct dirent *de;
...
int mds_kv_get(const char *key, int offset, void *entry, uint32_t *valuelen)
{
        ...
        if (S_ISREG(stbuf.st_mode)) {
                ret = __mds_kv_get_file(path, entry, valuelen);
                if (unlikely(ret))
                        GOTO(err_ret, ret);
        } else {
                ret = __mds_kv_get_dir(path, offset, entry, valuelen);
                if (unlikely(ret))
                        GOTO(err_ret, ret);
        }
...
int mds_kv_init()
{
        ...
        snprintf(path, MAX_LINE_LEN, "%s/mds",  SHM_ROOT);
